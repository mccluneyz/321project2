...
export class Level_4_2 extends Phaser.Scene {
  constructor() {
    super({ key: "Level_4_2" })
  }
  ...
  create() {
    ...
    this.sound.play("ocean_theme", { volume: 0.4, loop: true })
  }
  ...
  createTilemap() {
    // Use level_1_2 layout for level_4_2 with ocean tiles
    this.map = this.make.tilemap({ key: "level_1_2" })
  ...
  createDecorations() {
    ...
    decorPositions.forEach((pos, i) => {
      ...
      const debris = this.add.image(pos.x, pos.y, variant).setOrigin(0.5, 1)
      debris.setScale(0.5)
    })
  }
  ...
  createEnemies() {
    ...
    const enemyPositions = [
      { x: 10 * 64, y: 10 * 64, patrol: 128 },
      { x: 18 * 64, y: 8 * 64, patrol: 128 },
      { x: 26 * 64, y: 9 * 64, patrol: 128 },
      { x: 34 * 64, y: 7 * 64, patrol: 128 },
      { x: 14 * 64, y: 14 * 64, patrol: 192 },
      { x: 30 * 64, y: 14 * 64, patrol: 192 },
      { x: 38 * 64, y: 10 * 64, patrol: 128 }
    ]
    enemyPositions.forEach(pos => {
      const enemy = new PollutionEnemy(this, pos.x, pos.y, pos.patrol, 'ocean')
  ...
  setupCollisions() {
    ...
    if (this.player && this.player.sword) {
      this.physics.add.overlap(this.player.sword, this.enemies, (sword, enemy) => {
        if (enemy.isDying || !this.player.swordActive) return
        enemy.takeDamage(25, true)  // true = isSwordDamage
